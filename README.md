...
| `tamagotchi-plus` | Waveshare ESP32-S3-Touch-LCD-1.69 | Virtual pet for a 1.69" 240×280 touch display with an onboard IMU. Currently the base scaffold: display + CST816T touch bring-up, Wi-Fi provisioning (captive portal or Improv serial from the web flasher), OTA self-update from the GitHub Pages manifest, and battery % + USB-host status. The main screen is a 4× scaled, palette-indexed, strip-DMA pixel-art scene (sprites authored in Aseprite and converted at build time by `tools/sprites/`): it boots to an egg that cracks through six stages over a random 3–5 minutes of powered-on time, then hatches. See [`apps/tamagotchi-plus/README.md`](apps/tamagotchi-plus/README.md). |

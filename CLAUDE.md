...
> **media-room-dashboard's admin portal is ESPHome's stock `web_server` (v3, `local: true`), deliberately unprotected** to match cyd-clock and espframe (COM-307): no `auth:` on it and no password on `ota: platform: web_server` (the upload form) or `ota: platform: esphome`, so anyone on the LAN or the open setup AP can reflash it. `captive_portal` coexists with it (it only claims requests while the fallback AP is up). The stock v3 UI has no install control on the `update` entity, so the portal gets template `Check for Firmware Update` / `Install Firmware Update` buttons. Install deliberately isn't gated on `firmware_version == "dev"` (unlike `on_update_available`), so pressing it on a local/branch build replaces that build with the latest published release from master. The on-screen IP footer is driven by the `wifi_info` `ip_address` sensor's `on_value` and blanked in `wifi: on_disconnect`. A `template` text sensor with `update_interval: never` never publishes at all, so don't use that for constant diagnostics (it left media-room-dashboard's Firmware Version blank).
